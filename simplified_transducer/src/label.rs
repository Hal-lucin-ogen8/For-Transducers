    ...
    for (index, stmt) in stmts.iter().enumerate() {
        match stmt {
            ...
            Stmt::For0(var, inner_stmts) | Stmt::For1(var, inner_stmts) => {
                ...
                traverse_and_label(inner_stmts, path, labels, current_if.clone(), universe_formulas, for_vars, label_formulas);
                for_vars.pop();
                path.pop();
            }
            Stmt::If(if_expr, inner_stmts) => {
                ...
                traverse_and_label(inner_stmts, path, labels, Some(new_if_expr), universe_formulas, for_vars, label_formulas);
                path.pop();
            }
        }
    }
